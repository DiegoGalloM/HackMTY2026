...
test("tocar cualquier parte del reverso la regresa, menos los datos", async ({ page }) => {
  ...
  // Uno en la franja de arriba, fuera de los datos, sí voltea. Se clickea el
  // elemento y no una coordenada calculada: con la tarjeta inclinada su caja
  // envolvente es mayor que el plástico y el punto caería fuera.
  await page.locator(".card-back__stripe").click();
  await expect(trigger).toBeVisible();
  await expect(numero).toHaveCount(0);
});
