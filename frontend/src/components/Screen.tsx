 ...
 * de la barra inferior (altura + safe area) para que nada quede tapado.
 */
export default function Screen({ title, children }: ScreenProps) {
  return (
    <motion.main
      ...
      className="h-full overflow-y-auto pt-[var(--safe-top)] pb-[calc(var(--nav-height)+var(--safe-bottom)+1.5rem)]"
    >
      {title && (
