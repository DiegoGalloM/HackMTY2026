...
| `npm run e2e` | `playwright test` | Los 38 specs x 2 proyectos (desktop + movil). Levanta el dev server solo. |
...
```bash
cd frontend && npm run e2e       # 38 specs x 2 proyectos (1 skip en movil: hover)
```
...
(landing, página de pago, franja dentro de la app), login de una cuenta existente, el
